...
export function Icon({ name, className = "h-5 w-5" }: { name: string; className?: string }) {
  ...
  switch (name) {
    ...
    case "check":
      return (
        <svg viewBox="0 0 24 24" className={`${className} text-green-600`} fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
export function Card({ children, className = "" }: React.PropsWithChildren<{ className?: string }>) {
  return (
    <div className={`rounded-[28px] border border-white/75 bg-white shadow-[0px_32px_80px_-48px_rgba(15,23,42,0.35)] ${className}`}>{children}</div>
  );
}

type SidebarProps = { active?: "dashboard" | "schedule" | "appointments" | "patients" | "messages" };
export function Sidebar({ active = "dashboard" }: SidebarProps) {
  const router = useRouter();
  const [showLogoutModal, setShowLogoutModal] = React.useState(false);
  const primary = [
    { icon: "dashboard", label: "Dashboard", href: "/dashboard/doctor", key: "dashboard" },
    { icon: "calendar", label: "My Schedule", href: "/dashboard/doctor/schedule", key: "schedule" },
    { icon: "clock", label: "Appointments", href: "/dashboard/doctor/appointments", key: "appointments" },
    { icon: "users", label: "My Patients", href: "/dashboard/doctor/patients", key: "patients" },
    { icon: "inbox", label: "Messages / Inbox", href: "/dashboard/doctor/messages", key: "messages" },
    { icon: "shield", label: "Granted Access", href: "#" },
    { icon: "tasks", label: "Tasks / To-Do", href: "#" },
  ];
  ...
  <aside className="hidden w-72 flex-col justify-between rounded-[30px] border border-white/70 bg-gradient-to-br from-white via-white to-[#f1f5ff] p-5 shadow-[0px_36px_80px_-56px_rgba(15,23,42,0.4)] lg:flex">
      <div className="space-y-4">
        ...
        <nav className="space-y-1">
          {primary.map((item) => {
            const isActive = (item as typeof primary[number]).key === active;
            return (
              <Link
                key={item.label}
                className={`flex items-center gap-3 rounded-full px-3 py-3 text-sm font-medium transition ${isActive ? "bg-[#e6f0ff] text-[#1678ff]" : "text-gray-700 hover:bg-gray-50"}`}
                href={item.href}
              >
                <Icon name={item.icon} className={`h-5 w-5 ${isActive ? "text-[#1678ff]" : "text-gray-600"}`} />
      ...
      <div className="space-y-1">
        {secondary.map((item) => (
          <a key={item.label} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-gray-700 hover:bg-gray-50" href={item.href}>
            <Icon name={item.icon} className={item.icon === "star" ? "h-5 w-5 text-amber-400" : "h-5 w-5"} />
            <span>{item.label}</span>
          </a>
        ))}
        <button
